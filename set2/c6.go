...
func postUntilCorrectPrefix(data []byte) (cipher_text []byte) {
	fingerPrint := []byte("aaa1234567890abcdef1234567890abcdef")
	for true {
		cipher_text = post(append(fingerPrint, data...))
		if Equal(cipher_text[16:32], cipher_text[32:48]) {
			return cipher_text[48:]
		}
	}
	return cipher_text
}
...
func bruteByte(brutedBytes []byte) (bruted byte, err error) {
	...
	cipherBlock := postUntilCorrectPrefix([]byte(prefix))
	...
	for bruted := 0; bruted <= 255; bruted++ {
		...
		returnedBlock := postUntilCorrectPrefix(clearTextGuess)
