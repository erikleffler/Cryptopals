...
func httpHandlerPf(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		clearrText, err := crypt.B642Bytes(data.Secret)
		if err != nil {
			fmt.Println(err)
			return
		}
		clearrText = embeddData(clearrText)

		cipherText, err := crypt.Bytes2B64(crypt.CbcEncrypt(crypt.Pkcs7Pad(clearrText, 16),randIv, aesCipher))
		if err != nil {
			fmt.Println(err)
		}
...
func httpHandlerVal(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		clearrText, err := crypt.Bytes2B64(crypt.CbcDecrypt(cipherText, randIv, aesCipher))
		if err != nil {
			fmt.Println(err)
		}

		fmt.Fprintf(w, clearrText)
		return
