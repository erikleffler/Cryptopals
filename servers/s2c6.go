...
func randomPad(clear_text []byte) (padded []byte) {
	randPrefix := make([]byte, rand.Intn(15))
	rand.Read(randPrefix)
	return crypt.Pkcs7Pad(append(randPrefix, clear_text...), 16)
}
...
func http_handler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		cipher_text, err := crypt.Bytes2B64(crypt.EcbEncrypt(randomPad(clear_text), aesCipher))
		if err != nil {
			fmt.Println(err)
		}
